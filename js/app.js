...
window.renderMemberView = function () {
  ...
  document.getElementById('member-cards-grid').innerHTML = `
    <div class="member-card"><div class="member-card-icon">Fonds</div><div class="member-card-label">Fonds disponibles</div><div class="member-card-value">${solde.toLocaleString()} F</div><div class="member-card-sub">Banque scolaire</div></div>
    <div class="member-card ${current ? 'highlight' : ''}"><div class="member-card-icon">Bénéf.</div><div class="member-card-label">Bénéficiaire du jour</div><div class="member-card-value">${current ? current.adherentNom : '— Aucun —'}</div><div class="member-card-sub">${current ? current.type + ' — ' + (current.valeur ? Number(current.valeur).toLocaleString() + ' F' : '') : 'Pas de bénéficiaire actif'}</div></div>
    <div class="member-card"><div class="member-card-icon">Suivant</div><div class="member-card-label">Prochain en attente</div><div class="member-card-value">${next ? next.adherentNom : '— Aucun —'}</div><div class="member-card-sub">${next ? 'Rang #' + (next.ordre || 1) + ' — ' + next.type : 'File vide'}</div></div>
    <div class="member-card"><div class="member-card-icon">Mbr</div><div class="member-card-label">Total adhérents</div><div class="member-card-value">${state.adherents.length}</div><div class="member-card-sub">Membres actifs</div></div>`;
...
function renderBeneficiaires() {
  ...
  if (!state.beneficiaires.length) { el.innerHTML = '<div class="empty"><div class="empty-icon">Bénéf.</div>Aucun bénéficiaire</div>'; return; }
  const sorted = [...state.beneficiaires].sort((a, b) => (a.ordre || 0) - (b.ordre || 0));
...
export function updateDashboard() {
  ...
  if (!state.reunions.length) { dash.innerHTML = '<div class="empty"><div class="empty-icon">Réunion</div>Aucune réunion</div>'; return; }
...
function renderArticles() {
  ...
  el.innerHTML = params.articles.map(a => `
    <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:#f8fafc;border-radius:8px;margin-bottom:6px">
      ...
      <button class="btn btn-sm btn-danger" onclick="supprimerArticle('${a.id}')">Supprimer</button>
    </div>`).join('');
}
